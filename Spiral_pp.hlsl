...
{
	float3 colour1 = SceneTexture.Sample(PointSample, input.areaUV).rgb;
	float2 offsetUV;

	offsetUV = input.areaUV + float2(1 / gViewportWidth, 0);
	float3 Right = SceneTexture.Sample(PointSample, offsetUV).rgb;
	offsetUV = input.areaUV + float2(-1 / gViewportWidth, 0);
	float3 Left = SceneTexture.Sample(PointSample, offsetUV).rgb;
	offsetUV = input.areaUV + float2(0, -1 / gViewportHeight);
